...
try {
  ...
  await page.waitForFunction(previous=>document.querySelector('.slider-controls>span')?.textContent!==previous,initialReviewNumber,{timeout:8000})
  check(await page.locator('.review-autoplay-status').textContent()==='Reviews change automatically','homepage reviews: automatic carousel status is missing')
