...
export function AdditionalServicesPage() {
  ...
  const seo=route?.seo
  return <PageLayout title={seo?.title||approved.headline} description={seo?.description||approvedIntro.body} image={mediaUrl(seo?.social_image,image)} pageType="Service" mainClassName="additional-services-main">
    <PageHero eyebrow={route?.hero?.eyebrow||approved.title} title={route?.hero?.title||approved.headline} accent={route?.hero?.accent||'Coordinated with care.'} intro={route?.hero?.intro||approvedIntro.body} image={image} imageAlt={route?.hero?.image?.alt||'Surface preparation and repair work by Superior Plus Painting'} tone="terracotta"/>
    <TrustStrip/>
    <section className="inner-section additional-services-intro"><div className="container editorial-grid"><Reveal><SectionIntro eyebrow={approved.title} title="The work around painting" accent="matters just as much."/><p>{approvedIntro.body}</p></Reveal><Reveal className="additional-services-proof" delay={.1}><Hammer/><strong>Inspect</strong><span>Assess the existing condition</span><strong>Coordinate</strong><span>Sequence compatible trades and preparation</span><strong>Finish</strong><span>Complete the suitable coating system</span></Reveal></div><Divider color="#fbf6ec" variant="wave"/></section>
    <section className="inner-section cream additional-services-directory"><div className="container"><SectionIntro eyebrow="Approved service range" title="Repairs, preparation" accent="and property care." text="Tell us what you can see and what outcome you want. We’ll advise which services suit the project."/><div className="additional-service-grid">{services.map(([title,text],index)=>{const Icon=serviceIcons[index%serviceIcons.length];return <Reveal key={title} delay={(index%3)*.04}><article><span>{String(index+1).padStart(2,'0')}</span><div className="additional-service-icon"><Icon/></div><small>Property support</small><h3>{title}</h3><p>{text}</p></article></Reveal>})}</div></div></section>
    <section className="additional-service-band"><div className="container"><Reveal><Palette/><h2>{approvedNeed.heading}<br/><em>Tell us what you need.</em></h2></Reveal><Reveal delay={.1}><p>{approvedNeed.body}</p><button className="btn" onClick={()=>navigate('/contact')}>Discuss your project <ArrowRight/></button></Reveal></div></section>
    <TestimonialBand index={1}/><AreasBand/><ClosingCTA title={approvedQuote.heading} text={approvedQuote.body}/>
  </PageLayout>
}
...
export function AboutPage() {
  ...
  const fallbackApproach=[approvedQuality.body]
  ...
  return <PageLayout title={seo?.title||approved.title} description={seo?.description||approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="AboutPage" mainClassName="about-main">
    <PageHero {...hero}/>
    <TrustStrip/>
    <section className="inner-section about-service-range"><div className="container"><SectionIntro eyebrow={approvedWhat.heading} title="Complete painting care" accent="for the whole property." text={approvedWhat.body}/><div className="scope-grid">{approvedWhat.items.map((item,index)=>{const Icon=[PaintRoller,Palette,SprayCan,Trees,Hammer][index%5];return <Reveal key={item} delay={(index%5)*.04}><div className="scope-item scope-green"><small>{String(index+1).padStart(2,'0')}</small><span className="scope-icon"><Icon/></span><b>{item}</b></div></Reveal>})}</div></div></section>
    <section className="inner-section cream"><div className="container editorial-grid"><Reveal><SectionIntro eyebrow={approvedQuality.heading} title={fields.about_approach_title||'Quality begins'} accent="before the first coat."/>{approach.map(paragraph=><p key={paragraph}>{paragraph}</p>)}</Reveal><Reveal className="editorial-image" delay={.1}><img src={editorialImage} alt={fields.about_editorial_image?.alt||'Superior Plus painter preparing and painting a protected Melbourne interior'} loading="lazy" decoding="async"/><span>Superior Plus project</span></Reveal></div></section>
    <section className="inner-section about-roots"><div className="container brand-archive"><Reveal className="brand-archive-media"><img src={brandTeamArchive} alt="Superior Plus Painting branded work vehicle ready for a Melbourne project" loading="lazy" decoding="async"/><div className="brand-archive-stamp"><span>Melbourne painters</span><strong>Local work.<br/>Visible care.</strong><i>Residential · Commercial</i></div></Reveal><Reveal className="brand-archive-copy" delay={.1}><SectionIntro eyebrow={approvedRoots.heading} title="Built through" accent="hands-on service."/><p>{fields.about_roots_copy||approvedRoots.body}</p><small>A real Superior Plus work vehicle, equipped for painting projects across Melbourne.</small></Reveal></div></section>
    ...
    <TestimonialBand index={0}/><AreasBand/><ClosingCTA title={cta?.title||approvedClosing.heading} text={cta?.text||approvedClosing.body} label={cta?.link?.label} url={cta?.link?.url}/>
  </PageLayout>
}
...
export function ProcessPage() {
  ...
  const proof=textItems(fields.process_proof,approvedWhy.items)
  const proofDetails=[
  ...
  return <PageLayout title={seo?.title||approved.title} description={seo?.description||approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="HowTo" schemaData={{step:steps.map((item,index)=>({'@type':'HowToStep',position:index+1,name:item.title,text:item.text}))}}>
    ...
    <ClosingCTA title={cta?.title||approvedReady.heading} text={cta?.text||approvedReady.body} label={cta?.link?.label} url={cta?.link?.url}/>
  </PageLayout>
}
