...
export function AboutPage() {
  ...
  return <PageLayout title={seo?.title||'About Us'} description={seo?.description||'Meet Superior Plus Painting, Melbourne painting professionals committed to careful preparation, reliable service and quality workmanship.'} image={mediaUrl(seo?.social_image,hero.image)} pageType="AboutPage" mainClassName="about-main">
    <PageHero {...hero}/>
    <TrustStrip/>
...
export function ContactPage() {
  ...
  return <PageLayout title={seo?.title||'Get a Free Quote'} description={seo?.description||'Contact Superior Plus Painting for a free residential, commercial or property-painting quote across Melbourne.'} image={mediaUrl(seo?.social_image,hero.image)} pageType="ContactPage">
    ...
    <section className="contact-location"><div className="container contact-location-grid"><Reveal><div className="location-icon"><MapPin/></div><SectionIntro eyebrow="Our Melbourne location" title="Local to Chadstone." accent="Ready to come to you."/><address className="contact-street-address"><MapPin/>{mapAddress}</address><p>Superior Plus Painting services homes and businesses across Melbourne’s eastern and south-eastern suburbs from our Chadstone location.</p><a className="btn" href={mapUrl} target="_blank" rel="noreferrer">View address in Google Maps <ExternalLink size={17}/></a></Reveal><Reveal className="contact-map" delay={.1}><iframe src={mapEmbedUrl} title={`Superior Plus Painting at ${mapAddress}`} loading="lazy" referrerPolicy="no-referrer-when-downgrade"/></Reveal></div></section>
