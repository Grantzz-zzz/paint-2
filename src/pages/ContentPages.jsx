...
import { booleanValue, collectionFallbacks, fieldValue, mediaUrl, mergeContent, pairItems, textItems, useCollection, useEnquirySubmission, useRouteContent, useSiteContent } from '../content/ContentProvider'
...
export function ServicesPage() {
  ...
  return <PageLayout title={seo?.title||'Painting & Property Services'} description={seo?.description||'Explore Superior Plus Painting’s complete painting, preparation, repair and property improvement services across Melbourne.'} image={mediaUrl(seo?.social_image,hero.image)} pageType="CollectionPage" mainClassName="services-main">
    ...
    <ManagedPageExtras fields={fields} eyebrow="Service information"/>
...
export function AdditionalServicesPage() {
  ...
  const supporting=structuredSections(fieldValue(fields,'content_sections',undefined),[approvedNeed,approvedQuote])
  const need=supporting[0]??{title:'',paragraphs:[],items:[]}
  const quote=supporting[1]??structuredSections([approvedQuote])[0]
  ...
  return <PageLayout title={seo?.title??approved.headline} description={seo?.description??approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="Service" mainClassName="additional-services-main">
    ...
    {(need.title||need.paragraphs.length||need.items.length)&&<section className="additional-service-band"><div className="container"><Reveal><Palette/>{need.title&&<h2>{need.title}<br/><em>Tell us what you need.</em></h2>}</Reveal><Reveal delay={.1}>{need.paragraphs.map((paragraph,index)=><p key={`${paragraph}-${index}`}>{paragraph}</p>)}<CompactList items={need.items}/><button className="btn" onClick={()=>navigate('/contact')}>Discuss your project <ArrowRight/></button></Reveal></div></section>}
    {supporting.slice(2).map((section,index)=><ManagedContentSection section={section} index={index} defaultEyebrow="Property information" key={section.id}/>)}
    <TestimonialBand index={1}/><AreasBand/><ClosingCTA title={cta?.title??quote?.title} text={cta?.text??quote?.paragraphs?.join(' ')} label={cta?.link?.label} url={cta?.link?.url}/>
  </PageLayout>
}
...
export function AboutPage() {
  ...
  const sectionFallbacks=[approvedWhat,approvedQuality,approvedReasons,approvedRoots,approvedClosing]
  const rawManagedSections=fieldValue(fields,'content_sections',undefined)
  const legacySections=pairItems(rawManagedSections,sectionFallbacks.map(section=>[section.heading,[section.body,...(section.items??[])].filter(Boolean).join('\n')]))
  const managedSections=structuredSections(rawManagedSections,sectionFallbacks)
  const whatLines=(legacySections[0]?.[1]??'').split(/\r?\n/).map(line=>line.trim()).filter(Boolean)
  const whatTitle=managedSections[0]?.title??approvedWhat.heading
  const whatBody=managedSections[0]?.paragraphs?.[0]??whatLines[0]??''
  const whatItems=managedSections[0]?.items?.length?managedSections[0].items:(whatLines.length>1?whatLines.slice(1):approvedWhat.items)
  const qualityTitle=managedSections[1]?.title??approvedQuality.heading
  const qualityBody=managedSections[1]?.paragraphs?.join('\n\n')??approvedQuality.body
  const rootsTitle=managedSections[3]?.title??approvedRoots.heading
  const rootsBody=managedSections[3]?.paragraphs?.join('\n\n')??approvedRoots.body
  const closingTitle=managedSections[4]?.title??approvedClosing.heading
  const closingBody=managedSections[4]?.paragraphs?.join(' ')??approvedClosing.body
  ...
  const showStandards=booleanValue(fieldValue(fields,'about_standards_enabled',true),true)
  ...
  return <PageLayout title={seo?.title??approved.title} description={seo?.description??approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="AboutPage" mainClassName="about-main">
    <PageHero {...hero}/>
    <TrustStrip/>
    <section className="inner-section about-service-range"><div className="container"><SectionIntro eyebrow={whatTitle} title="Complete painting care" accent="for the whole property." text={whatBody}/><div className="scope-grid">{whatItems.map((item,index)=>{const Icon=paintingToolIcon(index);return <Reveal key={`${item}-${index}`} delay={(index%5)*.04}><div className="scope-item scope-green"><small>{String(index+1).padStart(2,'0')}</small><span className="scope-icon"><Icon/></span><b>{item}</b></div></Reveal>})}</div></div></section>
    <section className="inner-section cream"><div className="container editorial-grid"><Reveal><SectionIntro eyebrow={managedSections[1]?.eyebrow||qualityTitle} title={fieldValue(fields,'about_approach_title','Quality begins')} accent="before the first coat."/>{approach.map((paragraph,index)=><p key={`${paragraph}-${index}`}>{paragraph}</p>)}<CompactList items={managedSections[1]?.items}/></Reveal>{editorialImage&&<Reveal className="editorial-image" delay={.1}><img src={editorialImage} alt={fields.about_editorial_image?.alt??'Superior Plus painter preparing and painting a protected Melbourne interior'} loading="lazy" decoding="async"/><span>Superior Plus project</span></Reveal>}</div></section>
    ...
    {showStandards&&<section className="inner-section cream about-standards"><div className="container"><SectionIntro eyebrow="Why Superior Plus" title="Standards you can see." accent="Tap each card to look deeper." text="Each card pairs real project photography with a short service standard. Select a card to turn it over and read what that standard means during a project."/><FlipFeatureGrid items={standardCards} className="about-flip-grid"/></div></section>}
    {managedSections[4]?.items?.length>0&&<section className="inner-section managed-page-extra"><div className="container"><Reveal><CompactList items={managedSections[4].items}/></Reveal></div></section>}
    {managedSections.slice(5).map((section,index)=><ManagedContentSection section={section} index={index} defaultEyebrow="About Superior Plus" key={section.id}/>)}
...
export function ProcessPage() {
  ...
  const flexibleProcessSections=structuredSections(fieldValue(fields,'content_sections',undefined),[approvedWhy,approvedReady])
  const sectionProof=flexibleProcessSections[0]?.items?.length?flexibleProcessSections[0].items:(processSections[0]?.[1]?.split(/\r?\n/).map(item=>item.trim()).filter(Boolean)??approvedWhy.items)
  ...
  return <PageLayout title={seo?.title??approved.title} description={seo?.description??approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="HowTo" schemaData={{step:steps.map((item,index)=>({'@type':'HowToStep',position:index+1,name:item.title,text:item.text}))}}>
    ...
    {flexibleProcessSections[1]?.items?.length>0&&<section className="inner-section managed-page-extra"><div className="container"><Reveal><CompactList items={flexibleProcessSections[1].items}/></Reveal></div></section>}
    {flexibleProcessSections.slice(2).map((section,index)=><ManagedContentSection section={section} index={index} defaultEyebrow="Process information" key={section.id}/>)}
...
export function FAQsPage() {
  ...
  return <PageLayout title={seo?.title||'Frequently Asked Questions'} description={seo?.description||'Answers about quotes, service areas, preparation, timing, paint systems and booking with Superior Plus Painting.'} image={mediaUrl(seo?.social_image,hero.image)} pageType="FAQPage" mainClassName="faqs-main" schemaData={{mainEntity:items.map(item=>({'@type':'Question',name:item.question,acceptedAnswer:{'@type':'Answer',text:item.answer}}))}}>
    ...
    <ManagedPageExtras fields={fields} eyebrow="FAQ information"/>
...
export function ContactPage() {
  ...
  return <PageLayout title={seo?.title||'Get a Free Quote'} description={seo?.description||'Contact Superior Plus Painting for a free residential, commercial or property-painting quote across Melbourne.'} image={mediaUrl(seo?.social_image,hero.image)} pageType="ContactPage">
    ...
    <ManagedPageExtras fields={fields} eyebrow="Quote information"/><TestimonialBand index={2}/><AreasBand/>
  </PageLayout>
}
