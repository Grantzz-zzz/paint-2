...
export function AdditionalServicesPage() {
  ...
  return <PageLayout title={seo?.title??approved.headline} description={seo?.description??approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="Service" mainClassName="additional-services-main">
    ...
    {supporting.slice(2).map((section,index)=><section className={`inner-section managed-page-extra ${index%2?'cream':''}`} key={section.id}><div className="container"><Reveal><StructuredSectionCopy section={section} defaultEyebrow="Property information"/></Reveal></div></section>)}
...
export function AboutPage() {
  ...
  return <PageLayout title={seo?.title??approved.title} description={seo?.description??approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="AboutPage" mainClassName="about-main">
    ...
    {managedSections.slice(5).map((section,index)=><section className={`inner-section managed-page-extra ${index%2?'cream':''}`} key={section.id}><div className="container"><Reveal><StructuredSectionCopy section={section} defaultEyebrow="About Superior Plus"/></Reveal></div></section>)}
...
export function ProcessPage() {
  ...
  return <PageLayout title={seo?.title??approved.title} description={seo?.description??approvedIntro.body} image={mediaUrl(seo?.social_image,hero.image)} pageType="HowTo" schemaData={{step:steps.map((item,index)=>({'@type':'HowToStep',position:index+1,name:item.title,text:item.text}))}}>
    ...
    {flexibleProcessSections.slice(2).map((section,index)=><section className={`inner-section managed-page-extra ${index%2?'cream':''}`} key={section.id}><div className="container"><Reveal><StructuredSectionCopy section={section} defaultEyebrow="Process information"/></Reveal></div></section>)}
