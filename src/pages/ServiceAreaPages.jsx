...
import { fieldValue, mediaUrl, textItems, useCollection, useRouteContent } from '../content/ContentProvider'
...
export function ServiceAreasPage() {
  ...
  return <PageLayout title={seo?.title??'Service Areas — Melbourne’s Eastern Suburbs'} description={seo?.description??`Explore professional residential and commercial painting services across ${managedAreas.length} named Melbourne suburbs.`} image={mediaUrl(seo?.social_image,image)} pageType="CollectionPage" schemaData={{mainEntity:managedAreas.map(area=>({'@type':'Place',name:area.name,url:`/service-areas/${area.slug}`}))}}>
    ...
    <ManagedPageExtras
      fields={fields}
      eyebrow="Local painting coverage"
      accent="across Melbourne."
      imageAlt="Superior Plus Painting service area project"
    />
    ...
    <ClosingCTA title={route?.closing_cta?.title} text={route?.closing_cta?.text} label={route?.closing_cta?.link?.label} url={route?.closing_cta?.link?.url}/>
  </PageLayout>
}
...
export function ServiceAreaPage() {
  ...
  const routeArea=route?normalizeServiceArea({
    slug,
    name:fieldValue(fields,'area_name',fallbackArea?.name??route.title?.replace(/^Painters in\s+/i,'')),
    region:fieldValue(fields,'area_region',fallbackArea?.region),
    region_description:fieldValue(fields,'area_region_description',fallbackArea?.regionDescription),
    property_types:textItems(fieldValue(fields,'area_property_types',undefined),fallbackArea?.propertyTypes??[]),
    service_slugs:textItems(fieldValue(fields,'area_service_slugs',undefined),fallbackArea?.serviceSlugs??[]),
    local_context:fieldValue(fields,'area_local_context',fallbackArea?.localContext),
    neighbours:textItems(fieldValue(fields,'area_neighbour_slugs',undefined),fallbackArea?.neighbours??[]),
    card_image:fieldValue(fields,'area_card_image',fallbackArea?.cardImage),
  ...
  return <PageLayout title={seo?.title??pageTitle} description={seo?.description??description} image={mediaUrl(seo?.social_image,image)} pageType="Service" schemaData={{serviceType:'Painting services',areaServed:{'@type':'Place',name:area.name},availableChannel:{'@type':'ServiceChannel',serviceUrl:'/contact'}}}>
    ...
    <TrustStrip/>
    <section className="inner-section"><div className="container area-intro-grid"><Reveal><SectionIntro eyebrow={`Painting in ${area.name}`} title="The right preparation" accent="for the property."/> {area.localContext&&<p>{area.localContext}</p>}<p>Every project begins with an inspection and written quotation. The final scope depends on the surfaces, access, repairs, coating system and finish you want to achieve.</p></Reveal><Reveal className="area-property-card" delay={.1}><Home/><h3>Properties we can quote</h3>{(area.propertyTypes??[]).map(type=><span key={type}><Check/>{type}</span>)}</Reveal></div><Divider color="#fbf6ec" variant="wave"/></section>
    <ManagedPageExtras
      fields={fields}
      eyebrow={`${area.name} project details`}
      accent="prepared with care."
      imageAlt={`Superior Plus Painting project in ${area.name}`}
    />
    ...
    <ClosingCTA title={route?.closing_cta?.title??`Need painters in ${area.name}?`} text={route?.closing_cta?.text??`Tell us about your ${area.name} property, the surfaces involved and your preferred timing. We’ll confirm availability and arrange a free, no-obligation quotation.`} label={route?.closing_cta?.link?.label} url={route?.closing_cta?.link?.url}/>
  </PageLayout>
}
