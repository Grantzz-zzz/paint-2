...
import { PageLayout, PageHero, TrustStrip, SectionIntro, TestimonialBand, ClosingCTA } from '../components/PageLayout'
...
export default function ServicePage() {
  ...
  return <PageLayout title={seo?.title||`${page.title} Melbourne`} description={seo?.description||page.intro} pageType="Service" image={mediaUrl(seo?.social_image,image)}>
    ...
    <section className="inner-section scope-section"><div className="container">
      <SectionIntro eyebrow={labels.scope_eyebrow||"What we can help with"} title={page.scopeTitle} accent=""/>
    ...
    <section className="inner-section process-section"><div className="container">
      <SectionIntro eyebrow={labels.process_eyebrow||"How it comes together"} title={stepSection?.heading||labels.process_title||"Our Process"} accent=""/>
    ...
    {narrative.map((section,index)=><section className={`inner-section approved-copy-section ${index%2?'cream':''}`} key={section.heading}><div className="container"><SectionIntro eyebrow="Client-approved service information" title={section.heading} accent=""/><Reveal className="approved-copy-body"><div>{section.paragraphs?.map(paragraph=><p key={paragraph}>{paragraph}</p>)}</div>{section.items?.length?<ul>{section.items.map(item=><li key={item}><Check/>{item}</li>)}</ul>:null}</Reveal></div></section>)}
