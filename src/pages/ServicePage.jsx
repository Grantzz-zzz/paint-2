...
export default function ServicePage() {
  ...
  const {services,enabled}=useSiteContent()
  ...
  if (enabled && status==='loading') return <PageLayout title="Loading service" description="Loading the latest service information." pageType="Service"><section className="inner-section"><div className="container"><p>Loading the latest saved service content…</p></div></section></PageLayout>
  ...
  const image=page.image === undefined ? fallbackPage?.image : page.image
  const labels=page.sectionLabels||{}
  const headlineRule=serviceHeadlines[slug]
  const heroTitle=page.title === undefined && headlineRule ? headlineRule[1] : page.title
  return <PageLayout title={seo?.title??`${page.title} Melbourne`} description={seo?.description??page.intro} pageType="Service" image={mediaUrl(seo?.social_image,image)}>
    ...
    <AreasBand seed={slug}/>
    <ClosingCTA title={cta?.title??ctaSection?.heading??`Planning ${page.title?.toLowerCase()||'your project'}?`} text={cta?.text??ctaSection?.paragraphs?.join(' ')} label={cta?.link?.label} url={cta?.link?.url}/>
  </PageLayout>
}
