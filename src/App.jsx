...
const services = [
  { slug: 'residential-painting-melbourne', icon: Home, title: 'Residential Painting', tone: 'maroon', image: servicePages['residential-painting-melbourne'].image },
  { slug: 'commercial-painting-melbourne', icon: Building2, title: 'Commercial Painting', tone: 'green', image: servicePages['commercial-painting-melbourne'].image },
  { slug: 'interior-painting-melbourne', icon: PaintRoller, title: 'Interior Painting', tone: 'teal', image: servicePages['interior-painting-melbourne'].image },
  { slug: 'exterior-painting-melbourne', icon: SprayCan, title: 'Exterior Painting', tone: 'terracotta', image: servicePages['exterior-painting-melbourne'].image },
  { slug: 'roof-painting-melbourne', icon: Brush, title: 'Roof Painting', tone: 'maroon', image: roofHomepageCardImage.src, imagePosition: roofHomepageCardImage.position },
  { slug: 'fence-painting-melbourne', icon: Trees, title: 'Fence Painting', tone: 'terracotta', image: servicePages['fence-painting-melbourne'].image },
  ...
  { slug: 'wallpaper-removal-melbourne', icon: Brush, title: 'Wallpaper Removal', tone: 'teal', image: servicePages['wallpaper-removal-melbourne'].image },
  { slug: 'plaster-repairs-melbourne', icon: Hammer, title: 'Plaster Repairs', tone: 'cream', image: servicePages['plaster-repairs-melbourne'].image },
]
          ...
          >
            <span className="home-service-flip-inner">
              <span className="home-service-face home-service-front">
                <img src={service.image} alt={`${service.title} project by Superior Plus Painting`} style={{objectPosition:service.imagePosition||'center'}} loading="lazy" decoding="async"/>
...
function GuidesPreview({fields,items}) {
  ...
  const eyebrow=fieldValue(fields,'home_blog_eyebrow','From the blog')
  const title=fieldValue(fields,'home_blog_title','Plan with more')
  const accent=fieldValue(fields,'home_blog_accent','confidence.')
  const intro=fieldValue(fields,'home_blog_intro','Client-approved articles about repainting cycles, preparation, coating systems and choosing the right professional team for your property.')
  return <section className="section home-guides"><div className="container"><Reveal className="section-heading"><div>{eyebrow!==''&&<Eyebrow>{eyebrow}</Eyebrow>}<h2>{title}{accent&&<><br/><em>{accent}</em></>}</h2></div>{intro!==''&&<p>{intro}</p>}</Reveal><div className="home-guide-grid">{guides.map((guide,index)=><Reveal key={guide.slug} delay={index*.07}><article><button onClick={()=>navigate(`/blog/${guide.slug}`)}><img src={guide.image} alt={guide.imageAlt} loading="lazy" decoding="async"/><span><Clock3/>{guide.readTime}</span></button><small>{guide.category}</small><h3>{guide.title}</h3><p>{guide.excerpt}</p><button className="guide-link" onClick={()=>navigate(`/blog/${guide.slug}`)}>Read article <ArrowRight/></button></article></Reveal>)}</div><div className="section-action"><button className="btn" onClick={()=>navigate('/blog')}>Explore the painting blog <ArrowRight/></button></div></div><Divider color="#fbf6ec" variant="wave"/></section>
}
...
export function Testimonials({items,className='',profile=googleReviewProfile}) {
  ...
  const source='Google'
  const reviewProfile={...googleReviewProfile,...profile}
  const rating=Number(reviewProfile.rating)||googleReviewProfile.rating
