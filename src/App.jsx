...
function Contact({fields,business}) {
  ...
  return <section id="contact" className="contact section-track"><div className="contact-blob"/><div className="container contact-layout"><Reveal className="contact-copy"><Eyebrow light>Let’s talk colour</Eyebrow><h2>{fields?.home_quote_title||'Ready for a'}<br/><em>fresh start?</em></h2><p>{fields?.home_quote_text||'Tell us what you’re planning. We’ll arrange a free, no-obligation quote and help you choose the right way forward.'}</p><div className="contact-direct"><a href={business.phone_href}><span><Phone/></span><div><small>Call us</small><b>{business.phone_display}</b></div></a><a href={`mailto:${business.email}`}><span><Mail/></span><div><small>Email us</small><b>{business.email}</b></div></a></div></Reveal><Reveal delay={.15}><form className="quote-form" onSubmit={(e)=>{e.preventDefault();setSent(true)}}>{sent ? <motion.div initial={{opacity:0,scale:.95}} animate={{opacity:1,scale:1}} className="form-success"><span><Check/></span><h3>Thanks — we’ll be in touch.</h3><p>Your project details are ready to send. Connect this form to your preferred email service before launch.</p><button type="button" className="text-link" onClick={()=>setSent(false)}>Send another enquiry</button></motion.div> : <><div className="form-title"><span>Free quote request</span><small>{fields?.home_response_label||'Usually replies within 2 hours'}</small></div><div className="form-row"><label>Name<input required placeholder="Your name"/></label><label>Phone<input required type="tel" placeholder="04xx xxx xxx"/></label></div><div className="form-row"><label>Email<input required type="email" placeholder="you@email.com"/></label><label>Suburb<input required placeholder="Your suburb"/></label></div><label>Tell us about your project<textarea required rows="4" placeholder="What would you like painted?"/></label><button className="btn btn-wide" type="submit">Request my free quote <ArrowRight/></button><p className="form-note"><ShieldCheck/> No obligation. Your details stay private.</p></>}</form></Reveal></div></section>
}
